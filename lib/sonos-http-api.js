...
function HttpAPI(discovery, settings) {
  ...
  function handleAction(options, callback) {
    ...
    // parse slack payload string query
    var reg = new RegExp( '[?&]' + 'text' + '=([^&#]*)', 'i' );
    var string = reg.exec(options.action);
    var slackAction = string ? string[1] : null;

    if (!actions[slackAction]) {
      return callback({
        "response_type": "in_channel",
        "text": "Womp womp, the action _" + slackAction + "_ was not found"
      });
    }

    actions[slackAction](player, options.values, callback);

    if (!callback.invokeIntended) {
      return callback({
        "response_type": "in_channel",
        "text": "You preformed the _"+ slackAction + "_ action.",
      });
    }
  }
